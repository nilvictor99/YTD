<div class="p-6 dark:bg-primary dark:text-white">
    <h1 class="text-2xl font-bold mb-4 text-gray-900 dark:text-white">{{ __('downloader.title') }}</h1>

    <!-- Campo de URL -->
    <div class="mb-4">
        <label for="url"
            class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('downloader.url_label') }}</label>
        <input type="text" id="url" wire:model="url" placeholder="{{ __('downloader.url_placeholder') }}"
    ...
    <div class="mb-4">
        <label
            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('downloader.download_type_label') }}</label>
        <div class="flex space-x-6">
            <label class="inline-flex items-center cursor-pointer hover:opacity-75 transition-opacity">
                ...
                <span class="ml-2 text-gray-700 dark:text-gray-300 font-medium">
                    <i class="fas fa-video mr-1"></i> {{ __('downloader.video_option') }}
                </span>
            </label>
            <label class="inline-flex items-center cursor-pointer hover:opacity-75 transition-opacity">
                <input type="radio" wire:model="downloadType" value="audio"
                    class="form-radio h-5 w-5 text-green-600 dark:bg-gray-700 focus:ring-green-500 dark:focus:ring-green-600 border-gray-300 dark:border-gray-600">
                <span class="ml-2 text-gray-700 dark:text-gray-300 font-medium">
                    <i class="fas fa-music mr-1"></i> {{ __('downloader.audio_option') }}
                </span>
            </label>
        </div>
    </div>
    ...
    <div class="flex space-x-4 mb-4">
        <button wire:click="download" wire:loading.attr="disabled"
            class="px-4 py-2 bg-green-600 dark-text-white rounded-md hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-800 disabled:opacity-50">
            <span wire:loading.remove>{{ __('downloader.download_button') }}</span>
            <span wire:loading>
